...
// Stops movement and reverses state to prevent being stuck after
// move UP = 3 seconds
// move DOWN = 3 seconds
...
// with safety buffer time > than expected mangrove movement to prevent over turning.
...
long randUp;
long randDown;
long moveUp = 3000;         // 3 second motor movement UP
long moveDown = 3000;       // 3 second motormovement DOWN 
...
void loop() {
  ...
  // monitor PINMAIN main switch
  if (digitalRead(PINMAIN) == LOW && mainState == LOW)

    // if PINMAIN is active after a long time
  { if((millis() - timeDebounceMain) > delayDebounce)
  
    // time of PINMAIN activation = current time stamp
    { timeDebounceMain = millis();

      // main state is active, routine begins.
      mainState = HIGH;
      Serial.println("BEGIN");

      // routine happens when PINMAIN is activated
      // can be moved outside of the PINMAIN monitoring
      ...
      { 
        ...
        { if (millis() - timeDebounceRef < restUp)
          ...
          { if (millis() - timeDebounceRef - restUp < moveUp)
            { motorUp();
            }
            else
            { upState = LOW;
        ...
        { if (millis() - timeDebounceRef < restDown)
          ...
          { if (millis() - timeDebounceRef - restUp < moveDown)
            { motorDown();
            }
